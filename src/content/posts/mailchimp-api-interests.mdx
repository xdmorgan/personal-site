...
![Photo by Asa Rodger on Unsplash](./mailchimp-api-interests/asa-rodger-unsplash.jpg)
...
Next, we need the API Token created in the first section, as well as the List ID and Interest ID from the second. Replace `API_TOKEN`, `LIST_ID`, and `INTEREST_ID` in the constant definitions on lines 4–6.
