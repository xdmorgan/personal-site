...
With the foundation in place, we can now add a few more props to support many more use-cases.
...
To support traditional pagination we can add that functionality through composition. To do so, we'll wrap our `ListView` to create `PaginatedListView`.
...
Yeah, no problem. We can do this without modifying any internals and keep our pagination for free.
...
- Create an alternate pagination composition which uses an IntersectionOberserver-wrapped footer to support infinite scrolling
